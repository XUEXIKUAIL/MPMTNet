...
from .config import class_to_RGB, load_ckpt
# import cv2
...
def evaluate(logdir, save_predict=False):
    ...
    # print(test_loader.num_workers)
    # print(test_loader.collate_fn)

    model = get_model_t(cfg).to(device)
    ...
    # save_path_edge = os.path.join(logdir, 'predicts_edge')
    ...
    with torch.no_grad():
        ...
        for i, sample in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            predictout = predict[0].max(1)[1].cpu().numpy()  # [1, h, w]

            label = label.cpu().numpy()
            running_metrics_val.update(label, predictout)

            time_meter.update(time.time() - time_start, n=image.size(0))

            if save_predict:
                # print('开始保存预测图!')
                predict = predictout.squeeze(0)  # [1, h, w] -> [h, w]
...
def msc_evaluate(logdir, save_predict=False):
    ...
    print('inference time per image: ', time_meter.avg)
    print('inference fps: ', 1 / time_meter.avg)
    np.savetxt('evaluate.txt', time_meter.avg, 1 / time_meter.avg)

if __name__ == '__main__':
    import argparse

    parser = argparse.ArgumentParser(description="evaluate")
    parser.add_argument("--logdir", type=str, help="run logdir", default="run/.pth")
